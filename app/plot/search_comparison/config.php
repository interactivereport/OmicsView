...
$data = $DB -> get_row($sql);
$BXAF_CONFIG['COLNAME_LIST_COMPARISON'] = array();
foreach($data as $key => $value) {
	$BXAF_CONFIG['COLNAME_LIST_COMPARISON'][] = $key;
}

// Project Colnames
$sql = "SELECT * FROM `{$BXAF_CONFIG['TBL_PROJECTS']}`";
$data = $DB -> get_row($sql);
$BXAF_CONFIG['COLNAME_LIST_PROJECT'] = array();
foreach($data as $key => $value) {
	$BXAF_CONFIG['COLNAME_LIST_PROJECT'][] = $key;
}

// Gene Colnames
$sql = "SELECT * FROM `{$BXAF_CONFIG['TBL_GENECOMBINED']}`";
$data = $DB -> get_row($sql);
$BXAF_CONFIG['COLNAME_LIST_GENE'] = array();
foreach($data as $key => $value) {
	$BXAF_CONFIG['COLNAME_LIST_GENE'][] = $key;
}
...
// Sample Colnames
$sql = "SELECT * FROM `{$BXAF_CONFIG['TBL_SAMPLES']}`";
$data = $DB -> get_row($sql);
$BXAF_CONFIG['COLNAME_LIST_SAMPLE'] = array();
foreach($data as $key => $value) {
	$BXAF_CONFIG['COLNAME_LIST_SAMPLE'][] = $key;
}


// Default Table Columns
$BXAF_CONFIG['DEFAULT_TABLE_COLUMNS'] = array(
	'Comparison' => array('Comparison ID', 'DiseaseState', 'SampleSource', 'CellType'),
	'Sample' => array('Sample ID', 'PlatformName', 'CellType', 'DiseaseState'),
	'Gene' => array('GeneID', 'EntrezID', 'GeneName', 'Strand'),
	'Project' => array('Project ID', 'LandPlatforms', 'ExperimentType', 'Platform'),
);

// Total Number of Records
$BXAF_CONFIG['TOTAL_TABLE_COLUMNS'] = array(
	'Comparison' => '2870',
	'Sample' => '60268',
	'Gene' => '30373',
	'Project' => '950',
...
//-------------------------------------------------------------------------------
// Modify Functional Report Report
function modify_go_report($dir, $comp_index) {
  ...
  $table_content = substr($content, $pos_start + 31, $pos_end - $pos_start - 46);
    ...
    $htmlCode = "
      ...
      <link href='" . dirname($BXAF_CONFIG['BXAF_APP_URL']) . "/bxaf_lite/library/bootstrap/css/bootstrap.min.css.php' rel='stylesheet' type='text/css'>
      <link href='" . dirname($BXAF_CONFIG['BXAF_APP_URL']) . "/bxaf_lite/library/font-awesome/css/font-awesome.min.css' rel='stylesheet' type='text/css'>
      <link href='" . dirname($BXAF_CONFIG['BXAF_APP_URL']) . "/bxaf_lite/library/animate/animate.min.css.php' rel='stylesheet' type='text/css'>";
    $htmlCode .= '
      <script src="' . dirname($BXAF_CONFIG['BXAF_APP_URL']) . '/bxaf_lite/library/jquery/jquery.min.js"></script>
      <script src="' . dirname($BXAF_CONFIG['BXAF_APP_URL']) . '/bxaf_lite/library/bootstrap/js/bootstrap.min.js.php"></script>
      <script src="' . dirname($BXAF_CONFIG['BXAF_APP_URL']) . '/bxaf_lite/library/jquery/bootbox.min.js.php"></script>';
    $htmlCode .= "
    ...
    $htmlCode .= '
      <div class="container-fluid">
        <h3 class="font-raleway m-t-1">
          ...
          <a href="' . dirname($BXAF_CONFIG['BXAF_APP_URL']) . '/app/plot/search_comparison/single_comparison.php?type=comparison&id=' . $comp_index . '&inhouse=true" style="font-size:1rem;"><i class="fa fa-angle-double-right"></i> Back to comparison detail</a>
        </h3>
        <hr>
