...
if ($_GET['key'] != ''){
...
} elseif ($_GET['ProjectIndex'] > 0){
	$projectArray = get_one_record_by_id('Project', $_GET['ProjectIndex']);	
	...
	if ($projectArray['Job_ID'] > 0){
		...
		$dataArray['searchOption']	= 3;
		$dataArray['ProjectIDs'][0]	= $projectArray['ProjectID'];
		
		

		$submitHoldOff				= 1;
		
	} elseif ($projectArray['ProjectID'] != ''){
		$dataArray = array();
...
if ($_GET['GeneName'] != ''){
	$_GET['GeneName'] = guess_gene_name($_GET['GeneName'], '', 1);
}
...
if (array_size($jobInfo['Gene_Expression_Plot_Details']['Plot_Columns']) > 0){
	foreach($defaultGeneExpressionOptions as $currentColumn => $tempValue){
		if (in_array($currentColumn, $jobInfo['Gene_Expression_Plot_Details']['Plot_Columns'])){
			$defaultGeneExpressionOptions[$currentColumn]['Default'] = 1;
		} else {
			$defaultGeneExpressionOptions[$currentColumn]['Default'] = 0;
		}
	}
	
	foreach($defaultGeneExpressionOptionsCompleted as $currentColumn => $tempValue){
		if (in_array($currentColumn, $jobInfo['Gene_Expression_Plot_Details']['Plot_Columns'])){
			$defaultGeneExpressionOptionsCompleted[$currentColumn]['Default'] = 1;
		} else {
			$defaultGeneExpressionOptionsCompleted[$currentColumn]['Default'] = 0;
		}
	}
}
...
echo "<form id='form_application' action='javascript:void(0);' method='post' role='form' class='form-horizontal' enctype='multipart/form-data' autocomplete='off'>";
	...
	echo "<div id='inputSection'>";
			...
			echo "<div class='searchOptionRelated searchOptionRelated_0 {$class}'>";
				...
				echo "<div class='col-lg-12'>";
					echo "<div style='margin-top:20px;'>";
						echo "<p id='missingGeneNameSection' class='startHidden form-text'>" . 
								printFontAwesomeIcon('fas fa-exclamation-triangle text-danger') . 
								" Please enter at least a {$APP_MESSAGE['gene']} name and try again.</p>
						</div>";
				echo "</div>";
				echo "</div>";
			echo "</div>";
		...
		if (true){
			...
			echo "<div id='dataFilterSection' class='searchOptionRelated searchOptionRelated_0 dataFilterSection {$class}' style='margin-bottom:20px;'>";
				if (($searchOption == 0) && array_size($dataArray) > 0){
					...
					$_POST['mode'] 				= 1;
					$_POST['geneNames']			= $dataArray['GeneName'];
					...
					echo "<div class='row'>";
						echo "<div class='col-4'>";
							echo "<div style='padding-top:10px; padding-left:20px; padding-bottom:80px; min-height:209px; border:dotted 0px #999;'>";
			...
			if (array_size($APP_CONFIG['APP']['Module']['Gene-Expression-Comparison-Sample-DropDown']) > 0){
				...
				echo "<div class='col-lg-12'>";
					...
					foreach($APP_CONFIG['APP']['Module']['Gene-Expression-Comparison-Sample-DropDown'] as $currentColumn => $currentTitle){
						...
						$randomID 		= 'X' . md5($currentColumn . '::' .  rand());
		...
		if (true){
			...
			echo "<div class='col-lg-4 col-md-8 col-sm-12 col-xs-12'>";
				...
				echo "<div id='ProjectIDs_feedbackSection' xclass='startHidden'></div>";
	...
	echo "<div id='outputSection'>";
		...
		echo "<div class='col-lg-12'>";
			...
			echo "<div class='form-check'>";
				echo "<label class='form-check-label' for='Modified_DiseaseState_Enable'>";
					echo "<input class='form-check-input Modified_DiseaseState' type='checkbox' name='Modified_DiseaseState_Enable' id='Modified_DiseaseState_Enable' value='1' {$checked}>";
						echo "<strong>Update Disease State with Comparison Category</strong><span> (<a href='#Modified_DiseaseState_Modal' data-toggle='modal'>Help</a>)</span><strong>:</strong>";
					echo "</label>";
			echo "</div>";
			...
			if (true){
				...
				$modalTitle = "<h4 class='modal-title'>Modified Distate State with Comparison Category</h4>";
				$modalBody	.= "<div>";
					$modalBody .= "<p>If disease state is selected in the sample attributes section, the disease state value will be used by default. Some researchers prefer to split the sample data into two based on the comparison category. For example, suppose we have the following sample data:</p>";
					
					$modalBody .= "<p><strong>Disease State</strong>: Alzheimer's Disease (AD); <strong>Comparison Category</strong>: Disease vs. Normal</p>";
					
					$modalBody .= "<p>If this option is enabled, this sample data will be duplicated with different disease state values:</p>";
					
					$modalBody .= "<ol>
									<li><strong>Disease State</strong>: Alzheimer's Disease (AD)_Disease; <strong>Comparison Category</strong>: Disease</li>
									<li><strong>Disease State</strong>: Alzheimer's Disease (AD)_Normal; <strong>Comparison Category</strong>: Normal</li>
									</ol>";
									
					$modalBody .= '<p>If the "Display Abbreviation if Available" option is checked, the disease state values will become:</p>';
				...
				$modalBody .= "<div class='col-12'>";
					...
					if (true){
						...
						$modalBody .= "<div style='margin-top:12px;'>";
							$modalBody .= "<strong>Predefined Project Settings:</strong>";
						$modalBody .= "</div>";
			
						if (true){
							unset($checked, $class);
							if (isset($dataArray['JSON_Choice'])){
								if ($dataArray['JSON_Choice']){
									$checked = 'checked';
								}
							} else {
								$checked = 'checked';
							...
							$modalBody .= "<div class='form-check col-12' style='margin-left:15px;'>";
								$modalBody .= "<label for='JSON_Choice' class='form-check-label'>
													<input class='form-check-input inputForm' type='checkbox' id='JSON_Choice' name='JSON_Choice' value='1' {$checked}/>
													Enable Predefined Project Settings
												</label>";
								$modalBody .= "&nbsp;";
							$modalBody .= "</div>";
						}
						
						$modalBody .= "<p class='form-text'>Changes made below will have higher priority than the default project setting.</p>";
					...
					if (true){
						...
						$modalBody .= "<div style='margin-top:12px;'>";
							...
							$modalBody .= "<label for='{$name}' class='col-3 col-form-label'><strong>{$displayName}:</strong></label>";
							
							$modalBody .= "<div class='col-9'>";
								$modalBody .= "<select class='form-control' name='{$name}' id='{$name}' title='{$placeHolderText}'>";
								
									$modalBody .= "<option value=''>Auto</option>";
									
									unset($selected);
									if ($value == -1){
										$selected = 'selected';	
									}
									$modalBody .= "<option value='-1' {$selected}>None&#47;Disabled</option>";
									...
									foreach($defaultGeneExpressionOptionsCompleted as $currentColumn => $currentDetails){
										...
										$modalBody .= "<option value='{$currentColumn}' {$selected}>{$currentTitle}</option>";
						...
						$modalBody .= "<div style='margin-top:12px;'>";
							...
							$modalBody .= "<div class='col-9'>";
								$modalBody .= "<select class='form-control' name='{$name}' id='{$name}' title='{$placeHolderText}'>";
									...
									$modalBody .= "<option value=''>Auto (Same as the Attribute in Group Samples)</option>";
			...
			$modalBody .= "<div style='margin-top:12px;'>";
				...
				$modalBody .= "<div class='col-9'>";
					$modalBody .= "<select class='form-control' name='{$name}' id='{$name}' title='{$placeHolderText}'>";
						...
						$modalBody .= "<option value=''>None&#47;Disabled</option>";
	...
	if (true){
		...
		echo "<div id='submitButtonGroup' class='form-group  XsearchOptionRelated XsearchOptionRelated_1 XsearchOptionRelated_2 XsearchOptionRelated_3'>";
			echo "<br/>";
			...
			echo "<input type='hidden' id='JSON' name='JSON' value='{$dataArray['JSON']}'/>";
		echo "</div>";
	}
...
$(document).ready(function(){
	...
	$(document).on('click', '.refreshDataFilterTrigger', function(){
		loadDataFilterSection();
	});
	...
	$('#ProjectIDs').change(function(){
		...
		$('#ProjectIDs_feedbackSection').empty();
		...
		if (projectIDs != ''){
			...
			$.ajax({
				type: 'POST',
				url: 'app_project_ajax_gene_expression_single.php',
				data: data,
				success: function(responseText){
					responseText = $.trim(responseText);
					
					if (responseText != ''){
						$('#ProjectIDs_feedbackSection').html(responseText);
					}
				}
			});
		}
