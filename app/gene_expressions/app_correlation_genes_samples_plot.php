...
if ($resultFromCache['Summary']['otherOptions']['transform']){
	if ($resultFromCache['Summary']['otherOptions']['transform_value'] > 0){
		$xAxisNameHTML	= "Log<sub>2</sub>({$xAxisName} + {$resultFromCache['Summary']['otherOptions']['transform_value']})";
		$yAxisNameHTML	= "Log<sub>2</sub>({$yAxisName} + {$resultFromCache['Summary']['otherOptions']['transform_value']})";
		
		$xAxisName 		= "Log2({$xAxisName} + {$resultFromCache['Summary']['otherOptions']['transform_value']})";
		$yAxisName 		= "Log2({$yAxisName} + {$resultFromCache['Summary']['otherOptions']['transform_value']})";
	} else {
		$xAxisNameHTML	= "Log<sub>2</sub>({$xAxisName})";
		$yAxisNameHTML	= "Log<sub>2</sub>({$yAxisName})";
		
		$xAxisName 		= "Log2({$xAxisName})";
		$yAxisName 		= "Log2({$yAxisName})";
	}
	$correlation = round($resultFromCache['Correlation_Coefficient_Transformed'][$geneSourceIndex][$geneTargetIndex], 5);
} else {
	$xAxisNameHTML		= "{$xAxisName}";
	$yAxisNameHTML		= "{$yAxisName}";
...
if ($useSpearman){
	$tableContent['Header'][] = 'Sample ID';
	$tableContent['Header'][] = "<span class='xnowrap'>{$xAxisNameHTML} (Value)</span>";
	$tableContent['Header'][] = "<span class='xnowrap'>{$yAxisNameHTML} (Value)</span>";
	$tableContent['Header'][] = "<span class='xnowrap'>{$xAxisNameHTML} (Rank)</span>";
	$tableContent['Header'][] = "<span class='xnowrap'>{$yAxisNameHTML} (Rank)</span>";
...
$i = -1;
foreach($sourceValues as $sampleIndex => $sourceValue){
	$i++;
	$targetValue = $targetValues[$sampleIndex];
	if ($useSpearman){
		$varsString[] = '"' . $resultFromCache['Summary']['SampleIndex'][$sampleIndex] . '"';
		$dataString[] = '[' . $getCorrelationCoefficient_Spearman_Details['Output'][1][$i] . ',' . $getCorrelationCoefficient_Spearman_Details['Output'][2][$i] . ']';
				
		$tableContent['Body'][$sampleIndex]['Value'][] = "<a href='app_sample_review.php?ID={$sampleIndex}' target='_blank'>{$resultFromCache['Summary']['SampleIndex'][$sampleIndex]}</a>";
		$tableContent['Body'][$sampleIndex]['Value'][] = round($sourceValue, 5);
		$tableContent['Body'][$sampleIndex]['Value'][] = round($targetValue, 5);
		
		$tableContent['Body'][$sampleIndex]['Value'][] = $getCorrelationCoefficient_Spearman_Details['Output'][1][$i];
		$tableContent['Body'][$sampleIndex]['Value'][] = $getCorrelationCoefficient_Spearman_Details['Output'][2][$i];
	} else {
		$varsString[] = '"' . $resultFromCache['Summary']['SampleIndex'][$sampleIndex] . '"';
...
echo "<div class='row' id='rawData_{$geneSourceIndex}_{$geneTargetIndex}_section'>";
	echo "<div class='col-12' style='width:680px; height:680px; padding:10px;'>";
		echo "<canvas id='plotSection' width='660' height='660' xresponsive='true' aspectRatio='1:1'></canvas>";
	echo "</div>";
