...
$category					= 'Project';
$projectCount 				= getDashboardProjectDataCount($projectIndex);
$projectCountFormatted		= number_format($projectCount);


if (array_size($projectIndex) <= 0){
	...
	$text = "Summary of Selected Projects ({$projectCountFormatted})";	
	echo "<h3>{$text}</h3>";
	echo "<p>You are currently viewing a subset of projects. Please click <a href='app_dashboard_project.php'>here</a> to view all projects.</p>";

	//$projectCount 	= array_size(sample_index_to_project_id($projectIndex));
	//$projectCount	= array_size(sample_index_to_project_name($projectIndex));
}
...
$allCharts = $APP_CONFIG['Dashboard']['Charts_Project'];
		...
		echo "<div class='col-lg-6 col-md-12 col-sm-12 col-xs-12'>";
			echo "<div class='card'>";
				echo "<div class='card-header'>";
					...
					echo "<span class='selectedCountIndividualSection startHidden' style='padding-left:10px;'>(Selected: <span class='selectedCount'></span> out of {$projectCountFormatted})</span>";
...
echo "<div id='dashboardSection'  style='padding-left:5px;'>";
	...
	echo "<input type='hidden' id='selected_project_indexes' value=''/>";
...
#chartSection_StudyType .x.axis text {
...
$(document).ready(function(){
	...
	d3.csv('app_dashboard_project_data.php?projectIndex=<?php echo $_GET['projectIndex']; ?>', function (data) {
        var totalWidth 	= 990;
		...
		?>
			...
			function removeEmptyValues_<?php echo $currentChart['Column']; ?>(sourceGroup){
				return {
					all:function(){
						return sourceGroup.all().filter(function(d){
							<?php
								...
								if ($APP_CONFIG['Dashboard']['Charts_Project'][$tempKey]['Others'] != ''){
									$checks[] = "(d.key != '{$APP_CONFIG['Dashboard']['Charts_Project'][$tempKey]['Others']}')";
								}
        ...
        var recordCount = dc.dataCount('#selectedCountSection')
            ...
            .group(all)
            .html({
                some: "<strong>%filter-count</strong> out of <strong><?php echo $projectCountFormatted; ?></strong> projects have been selected. <br/> <a href='javascript:dc.filterAll(); dc.renderAll();'><?php echo printFontAwesomeIcon('fas fa-sync-alt'); ?> Reset all filters</a> &nbsp; <a href='javascript:void(0);' class='dashboardTrigger'><?php echo printFontAwesomeIcon('fas fa-chart-pie'); ?> Create a new dashboard based on selected projects</a>",
                all: 'Showing all projects (<?php echo $projectCountFormatted; ?>). Please click on the graphs to apply filters.<br/><br/>'
            });
	...
	$(document).on('click', '.dashboardTrigger', function(){
		...
		var selected_project_indexes = $('#selected_project_indexes').val();

		if (selected_project_indexes != ''){
			
			var data = new Object();	
			data['index[]']	= selected_project_indexes.split(',');
			
			$.ajax({
				type: 'POST',
				url: 'app_list_ajax.php?action=12',
				data: data,
...
function reloadTable(obj){
	...
	for (var tempKey in obj) {
		...
		data['ProjectIndex'].push(obj[tempKey].ProjectIndex);
		selected_project_indexes += ',' + obj[tempKey].ProjectIndex;
	}
	
	$('#selected_project_indexes').val(selected_project_indexes);
	

	$.ajax({
		type: 'POST',
		url: 'app_dashboard_project_table_exe.php',
		data: data,
		success: function(responseText){
