...
if (isset($_GET['action']) && $_GET['action'] == 'get_project_samples') {
	...
	// Private projects and samples
	$sql = "SELECT `ProjectIndex` FROM `App_User_Data_Projects` WHERE `ProjectID` = ?s";
	$ProjectIndex = $BXAF_MODULE_CONN -> get_one($sql, $_GET['projectname']);

	$sql = "SELECT `SampleID` FROM `App_User_Data_Samples` WHERE `ProjectIndex` = ?i ORDER BY `SampleID`";
	$samples = $BXAF_MODULE_CONN -> get_col($sql, $ProjectIndex);
...
<?php
	...
	$sql = "SELECT `ProjectIndex`, `ProjectID` FROM `App_User_Data_Projects` WHERE `User_ID` = " . $_SESSION['BXAF_USER_LOGIN_ID'] . " OR `Permission` = 1 ORDER BY `ProjectID`";
	$projects = $BXAF_MODULE_CONN -> get_assoc('ProjectIndex', $sql );

	$sql2 = "SELECT `ProjectIndex`, COUNT(*) FROM `App_User_Data_Samples` WHERE `User_ID` = " . $_SESSION['BXAF_USER_LOGIN_ID'] . " OR `Permission` = 1 GROUP BY `ProjectIndex`";
	$project_samples = $BXAF_MODULE_CONN -> get_assoc('ProjectIndex', $sql2 );

	foreach($projects as $projectindex => $ProjectName){
		$count = array_key_exists($projectindex, $project_samples) ? $project_samples[$projectindex] : 0;
		if($count <= 0) continue;
		echo "<tr>";
			echo "<td>$ProjectName</td>";
			echo "<td>" . $count . "</td>";
    ...
    $(document).ready(function() {
      	...
      	$(document).on('click', '#btn_select_sample', function() {
			...
			if( $('#modal_select_sample_initiated').val() == '' ){
				...
				$('#table_samples').DataTable({
			        ...
			        "serverSide": true,
			        "ajax": {
			            "url": "../tool_data_view/modal_sample.php?action=get_sample_list<?php if($_SERVER['QUERY_STRING'] != '') echo '&' . $_SERVER['QUERY_STRING']; ?>",
			            "type": "POST"
			        },
			        "columns": [
						{ "data": "SampleID" },
		...
		$(document).on('click', '#btn_select_project', function() {
      	...
      	$(document).on('click', '.btn_select_current_project', function() {
			$.ajax({
    			type: 'GET',
    			url: '../tool_data_view/modal_sample.php?action=get_project_samples&projectname=' + $(this).attr('projectname'),
        ...
        $(document).on('click', '.btn_saved_sample_lists', function() {
            ...
            $.ajax({
    			type: 'GET',
    			url: '../tool_data_view/modal_sample.php?action=get_saved_lists&category=' + category,
    			success: function(responseText){
