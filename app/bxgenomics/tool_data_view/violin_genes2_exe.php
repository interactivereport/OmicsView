...
// echo "<pre>" . print_r($_POST, true) . "</pre>"; exit();
	...
	// Private
    $sql = "SELECT `SampleIndex`, `ProjectIndex` FROM `App_User_Data_Samples` WHERE `SampleIndex` IN (?a)";
    $sampleindex_projectindex_private = $BXAF_MODULE_CONN -> get_assoc('SampleIndex', $sql, array_keys($sample_indexnames));
	...
	foreach($gene_indexnames as $gene_index=>$gene_name){
        ...
        foreach($sampleindex_projectindex_private as $sample_index=>$project_index){
            $gene_index1 = $gene_index + 1;
            fwrite($fp, "{$sample_index}\t{$gene_index1}\t{$gene_index1}\n");
		}
	}
    ...
    foreach($projectindex_sampleindex as $projectindex=>$sampleindexes){
        ...
        $batch_commands .= "{$BXAF_CONFIG['TABIX_BIN']} {$tabix_index_file} -R {$fileInput} >> {$fileOutput}\n";
    }
    ...
    if (($handle = fopen($fileOutput, "r")) !== FALSE) {
        while (($data = fgetcsv($handle, 0, "\t")) !== FALSE) {
            ...
            // $tabix_results[ $data[0] ][ $data[1] ] = log($data[2] + 0.5, 2);
    ...
    if (($handle = fopen($meta_data_file, "r")) !== FALSE) {
        ...
        while (($data = fgetcsv($handle, 0, "\t")) !== FALSE) {
            ...
            foreach($meta_data_head as $i=>$c){
                // $meta_data_info[$sample_name][$c] = $data[$i];
    ...
    $Rscript = <<< RSCRIPT_CONTENT
        ...
        subdata=read.csv(data_file,row.names=1, header=T, check.names=F);
        subdata[subdata<0]=0;
        subdata=log2(subdata+0.5);
        samples=read.csv(sample_file);
        ID_order=match(colnames(subdata), samples[, 1])
        samples=samples[ID_order, ]
        ...
        p<-ggplot(Pdata, aes(x=Cluster, y=value, color=Cluster, fill=Cluster)) +geom_violin(trim=T) + scale_x_discrete(limits = rev(levels(as.factor(Pdata\$Cluster))))+
         ...
         theme(axis.text.x = element_text(angle = 90, hjust = 1)) +labs(x=meta_type, y="log2(Value+0.5)", title="Gene")+theme(legend.position="none");
