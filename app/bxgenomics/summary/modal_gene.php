...
<script>
    ...
    $(document).ready(function() {
		...
		$(document).on('click', '.btn_show_content_detail',function(){
			...
			$.ajax({
				url: '../summary/modal_gene.php?action=get_geneset_detail&id=' + geneset_id,
        ...
        $(document).on('click', '.btn_select_current_geneset',function(){
			...
			$.ajax({
				url: '../summary/modal_gene.php?action=get_geneset_genes&id=' + geneset_id,
				success: function(responseText, statusText){
      	...
      	$(document).on('click', '.btn_select_geneset', function() {
			...
			if( $('#modal_select_geneset_initiated').val() == '' ){
				...
				$('#table_geneset').DataTable({
			        ...
			        "serverSide": true,
			        "ajax": {
			            "url": "../summary/modal_gene.php?action=get_geneset_list<?php if($_SERVER['QUERY_STRING'] != '') echo '&' . $_SERVER['QUERY_STRING']; ?>",
			            "type": "POST"
			        },
			        "columns": [
						{ "data": "ID" },
        ...
        $(document).on('click', '.btn_saved_gene_lists', function() {
            ...
            $.ajax({
    			type: 'GET',
    			url: '../summary/modal_gene.php?action=get_saved_lists&category=' + category,
