...
if(isset($_GET['action']) && $_GET['action'] == "search_page") {
	...
	$z_score = 'neg';
	if(isset($_POST['Z_Score']) && $_POST['Z_Score'] == 'pos') $z_score = 'pos';


	$dir = $BXAF_CONFIG['WORK_DIR'] . 'bxgenomics/comparison_page/page_out';
	...
	$sql = "SELECT COUNT(*) FROM `tbl_page_genesets`";
	$msigdb_records = $BXAF_MODULE_CONN->get_one($sql);
	...
	foreach ($internal_comparison_id_names as $key => $values) {
		...
		if (is_array($accessible_project_indexes) && array_key_exists($project_index, $accessible_project_indexes) ){
			$comparison_id_names[ $key ] = $values['ComparisonID'];
		}
	}
	...
	$key = bxaf_save_to_cache($names);
	$table_contents .= "<div class='my-3 px-5'><a href='Javascript: void(0);' class='btn_save_selected_records mx-2'><i class='fas fa-angle-double-right'></i> Save Selected Comparisons</a> <a target='_blank' href='../../gene_expressions/app_list_new.php?Category=Comparison&key=$key' class='mx-2'><i class='fas fa-angle-double-right'></i> Save All Comparisons</a> <a href='Javascript: void(0);' class='btn_save_selected_records_plus_target mx-2'><i class='fas fa-angle-double-right'></i> Show selected comparisons in Pathway Heatmap</a> </div>";

	$table_contents .= "<div class='w-100'>";

	$table_contents .= "<table id='table_search_results' class='table table-bordered table-hover'>";
	$table_contents .= "<thead><tr class='table-success'><tr class='table-success'><th><input type='checkbox' class='checkbox_check_all' onChange=\"$('.checkbox_check_individual').prop('checked', \$(this).prop('checked') );\" /></th><th>Found Comparisons</th><th># Terms in Comparison</th><th># Overlaps</th><th>Enrichment Score</th><th>Matches</th></tr></thead><tbody>";

	foreach($overlap_counts as $comparison=>$count){

		if(100 * $count / count($target_list) < intval($_POST['Minimum_Overlaps'])) continue;

		$score = sprintf("%.4f", ($count / $found_counts[$comparison]) / (count($target_list) / $msigdb_records) );
...
if(isset($_GET['action']) && $_GET['action'] == "search") {
	...
	$sql = "SELECT COUNT(*) FROM `$db_table` WHERE `GO_Tree` = ?s";
	$go_tree_records = $BXAF_MODULE_CONN->get_one($sql, $_POST['GO_Tree']);
	...
	$found_terms = array();
	$found_terms1 = array();
	if($_POST['Found_Direction'] == '1'){
		$sql = "SELECT `Comparison_Name`, `Terms` FROM `$db_table` WHERE `GO_Tree` = ?s AND `Direction` = ?s";
		if($_POST['Direction'] == 'Up') {
			$found_terms = $BXAF_MODULE_CONN->get_assoc('Comparison_Name', $sql, $_POST['GO_Tree'], 'Up');
		}
		else {
			$found_terms1 = $BXAF_MODULE_CONN->get_assoc('Comparison_Name', $sql, $_POST['GO_Tree'], 'Down');
		}
	}
	else if($_POST['Found_Direction'] == '-1'){
		$sql = "SELECT `Comparison_Name`, `Terms` FROM `$db_table` WHERE `GO_Tree` = ?s AND `Direction` = ?s";
		if($_POST['Direction'] == 'Up') {
			$found_terms1 = $BXAF_MODULE_CONN->get_assoc('Comparison_Name', $sql, $_POST['GO_Tree'], 'Down');
		}
	...
	$overlap_counts1 = array();
	$overlap_details1 = array();
	foreach($found_terms1 as $comparison=>$terms){
		if($comparison == $target_info['Comparison_Name']) continue;

		$terms = explode(",", $terms);
		$found_counts1[$comparison] = count($terms);
	...
	$sql = "SELECT `ComparisonID`, `ComparisonIndex` FROM `Comparisons`";
	$comparison_nameindex = $BXAF_MODULE_CONN->get_assoc('ComparisonID', $sql);
	...
	$key = bxaf_save_to_cache(array_keys($overlap_counts));
	$table_contents .= "<div class='my-3 px-5'><a href='Javascript: void(0);' class='btn_save_selected_records mx-2'><i class='fas fa-angle-double-right'></i> Save Selected Comparisons</a> <a target='_blank' href='../../gene_expressions/app_list_new.php?Category=Comparison&key=$key' class='mx-2'><i class='fas fa-angle-double-right'></i> Save All Comparisons</a> <a href='Javascript: void(0);' class='btn_save_selected_records_plus_target mx-2'><i class='fas fa-angle-double-right'></i> Show selected comparisons in Pathway Heatmap</a> </div>";
	...
	if($_POST['Show_Terms'] == 1) $table_contents .= "<thead><tr class='table-success'><th><input type='checkbox' class='checkbox_check_all' onChange=\"$('.checkbox_check_individual').prop('checked', \$(this).prop('checked') );\" /></th><th>Found Comparisons</th><th># Terms in Comparison</th><th># Overlaps</th><th>Enrichment Score</th><th>Matches</th><th>Overlapped Terms</th></tr></thead><tbody>";
	else $table_contents .= "<thead><tr class='table-success'><th><input type='checkbox' class='checkbox_check_all' onChange=\"$('.checkbox_check_individual').prop('checked', \$(this).prop('checked') );\" /></th><th>Found Comparisons</th><th># Terms in Comparison</th><th># Overlaps</th><th>Enrichment Score</th><th>Matches</th></tr></thead><tbody>";


	foreach($overlap_counts as $comparison=>$count){

		if(100 * $count / count($selected_terms) < intval($_POST['Minimum_Overlaps'])) continue;


		$score = sprintf("%.4f", ($count / $found_counts[$comparison] ) / ( count($selected_terms) / $go_tree_records) );

		$table_contents .= "<tr>";

			$table_contents .= "<td><input type='checkbox' class='checkbox_check_individual' value='" . $comparison . "' /></td>";
			$table_contents .= "<td>" . $comparison . " ";

			$table_contents .= " <a class='mx-2' title='Show Comparison Details' href='../../plot/search_comparison/single_comparison.php?type=comparison&id=" . $comparison_nameindex[$comparison] . "' target='_blank'><i class='fas fa-list'></i></a>";
			$table_contents .= " <a class='mx-2' title='Show Vocano Plot' href='../../plot/volcano/index.php?id=" . $comparison_nameindex[$comparison] . "' target='_blank'><span class='badge badge-pill table-success text-danger'>V</span></a>";
			$table_contents .= " <a class='mx-2' title='Show WikiPathway' href='../tool_pathway/index.php?id=" . $comparison_nameindex[$comparison] . "' target='_blank'><span class='badge badge-pill table-success text-danger'>W</span></a>";
			$table_contents .= " <a class='mx-2' title='Show Reactome Pathway' href='../tool_pathway/reactome.php?id=" . $comparison_nameindex[$comparison] . "' target='_blank'><span class='badge badge-pill table-success text-danger'>R</span></a>";
			$table_contents .= " <a class='mx-2' title='Show KEGG Pathway' href='../tool_pathway/kegg.php?id=" . $comparison_nameindex[$comparison] . "' target='_blank'><span class='badge badge-pill table-success text-danger'>K</span></a>";

			$Found_Direction = 'Match';
			$Direction2 = 'Up';
			if($_POST['Direction'] == 'Up'){
				$Found_Direction = 'Match';
			}
			else if($_POST['Direction'] == 'Down'){
				$Found_Direction = 'Anti-Match';
			}

			$table_contents .= " <a title='Show Venn Diagrams' href='comparison_compare.php?comparison1=" . $_POST['comparison1'] . "&comparison2=" . $comparison . "&GO_Tree=" . $_POST['GO_Tree'] . "&Direction1=" . $_POST['Direction'] . "&Direction2=" . $Direction2 . "&P_Value_Cutoff=" . $_POST['P_Value_Cutoff'] . "' target='_blank'><i class='fas fa-adjust'></i></a>";
			$table_contents .= "</td>";
			...
			$table_contents .= "<td>" . $Found_Direction . "</td>";
	...
	foreach($overlap_counts1 as $comparison=>$count){
		...
		$score = sprintf("%.4f", ($count / $found_counts1[$comparison] ) / ( count($selected_terms) / $go_tree_records) );
...
if(isset($_GET['action']) && $_GET['action'] == "show_venn_diagram") {
	...
	foreach($comparison_names as $i=>$name){
		...
		if(trim($_POST['comparison' . ($i+1)]) != ''){
			$comparisons[$i] = trim($_POST['comparison' . ($i+1)]);
		}
	}
	...
	if(isset($_GET['type']) && $_GET['type'] == "go_results") {
		...
		$name_result = array();
		foreach($go_info as $i=>$row){
			$j = $comparisons_flip[ $row['Comparison_Name'] ];
			$name_result[$i] = $comparison_names[$j];
			$value_result[ $i ] = explode(',', $row['Terms']);
		}
		...
		foreach($comparisons as $i=>$c){
			...
			$name_result[$i] = $comparison_names[$i];
			$value_result[ $i ] = array();

			$z_score = 'neg';
			if(isset($_POST['Z_Score' . ($i+1)]) && $_POST['Z_Score' . ($i+1)] == 'pos') $z_score = 'pos';
...
if(isset($_GET['action']) && $_GET['action']=="save_to_cache"){

	$records = json_decode($_POST['records']);

	echo bxaf_save_to_cache($records);
